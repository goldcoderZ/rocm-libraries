...
TEST_F(TestRMSNormBackwardOperationDescriptor, SetComputeDataTypeWrongElementCount)
{
    ...
    ASSERT_THROW_HIPDNN_STATUS(
        desc->setAttribute(
            HIPDNN_ATTR_RMSNORM_BACKWARD_MATH_PREC_EXT, HIPDNN_TYPE_DATA_TYPE, 2, &computeType),
        HIPDNN_STATUS_BAD_PARAM);
}
...
TEST_F(TestRMSNormBackwardOperationDescriptor, GetAttributeComputeType)
{
    ...
    int64_t elementCount = 0;
    ASSERT_NO_THROW(desc->getAttribute(HIPDNN_ATTR_RMSNORM_BACKWARD_MATH_PREC_EXT,
