...
struct MmaPipelineBase
{
     ...
     * @tparam TupleArgs  Parameter pack deduced from the (A, B, C, [scaleA, scaleB]) tuple after @c
     * execImpl; only C is consumed. Must be either a 3-element or 5-element pack:
     *
     *                    | Index | 0 | 1 | 2 | 3      | 4      |
     *                    |-------|---|---|---|--------|--------|
     *                    | 3-arg | A | B | C |   —    |   —    |
     *                    | 5-arg | A | B | C | scaleA | scaleB |
     *
     *                    Only element 2 (C) is consumed by this function.
     *
     * @return The final D output in the user-facing vector type.
     */
    template <typename... TupleArgs>
    CK_TILE_DEVICE static auto applyTransformToOutput(std::tuple<TupleArgs...>&& vecs)
    {
        auto&& c_result = std::get<2>(vecs);
